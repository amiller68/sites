...
allowed-tools:
  - mcp__linear-kro__get_issue
  - mcp__linear-kro__list_comments
  - mcp__linear-kro__create_comment
  - mcp__linear-kro__list_issues
  - Read
  - Glob
  - Grep
---
...
**IMPORTANT**: This command uses `mcp__linear-kro__*` functions. Only work on issues in the **"sites"** project.

## Steps

1. If no issue ID provided in `$ARGUMENTS`, ask the user for the issue ID.

2. Fetch issue details using `mcp__linear-kro__get_issue`:
   - Get title, description, acceptance criteria
   - Note the current state and assignee
   - **Verify the issue belongs to the "sites" project** - if not, inform the user and stop

3. Get existing comments using `mcp__linear-kro__list_comments`:
